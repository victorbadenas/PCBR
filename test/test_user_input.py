...
class TestUserRequest(unittest.TestCase):
    ...
    def test_preferences(self):
        ...
        feature_relevance_scaled, preferences_scaled = UserRequest._process_preferences(self.dur, self.preferences_str,
                                                                                        feature_relevance_matrix)
        self.assertTrue(np.isclose(feature_relevance_scaled, np.full((13,), 0.51553254)).all())
        self.assertTrue(
            (preferences_scaled == np.array([1., 0.25, 0.5, 0., 0.25, 0., 0.5, 0.75, 1., 0., 1., 0., 0.])).all())
